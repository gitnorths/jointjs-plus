...
export class SymbolMakerWindow extends BrowserWindow {
  constructor () {
    super({
      // @ts-ignore
      icon: path.join(__static, 'logo/symbolMaker.png'),
      width: 1600,
      height: 900,
      show: false,
      webPreferences: {
    ...
    if (process.env.WEBPACK_DEV_SERVER_URL) {
      ...
      this.loadURL(process.env.WEBPACK_DEV_SERVER_URL + 'symbolMaker')
      this.webContents.openDevTools()
    } else {
      createProtocol('app')
      // Load the index.html when not in development
      this.loadURL('app://./symbolMaker.html')
    }

    this.on('close', (e) => {
      this.webContents.send('sm-window-close')
      e.preventDefault()
    })
