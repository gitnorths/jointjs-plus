...
import { CustomGroupItemEntity, SymbolBlockVarOutputInstEntity } from '@/service/entity/deviceEntities'
import { CustomGroupItem } from '@/model/dto'
import { EnableStatusEnum } from '@/model/enum'

export class CustomGroupService {
  public static async openCustomGroup (obj: any, dbName: string) {
    const result: CustomGroupItem[] = []
    if (!obj) {
      return result
    }
    const dataSource = DataSourceManager.getDataSource(dbName)
    if (!dataSource) {
      throw new Error(`自定义分组打开失败，数据源${dbName}不存在`)
    }
    const itemEntities = await dataSource.getRepository(CustomGroupItemEntity).find({
    ...
    for (const itemEntity of itemEntities) {
      const item = new CustomGroupItem(itemEntity)
