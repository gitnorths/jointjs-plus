...
import { ReportGroupItemEntity, SymbolBlockVarOutputInstEntity } from '@/service/entity/deviceEntities'
import { ReportGroupItem } from '@/model/dto'
import { EnableStatusEnum } from '@/model/enum'

export class ReportGroupService {
  public static async openReportGroup (obj: any, dbName: string) {
    const result: ReportGroupItem[] = []
    if (!obj) {
      return result
    }
    const dataSource = DataSourceManager.getDataSource(dbName)
    if (!dataSource) {
      throw new Error(`报告分组打开失败，数据源${dbName}不存在`)
    }
    const itemEntities = await dataSource.getRepository(ReportGroupItemEntity).find({
    ...
    for (const itemEntity of itemEntities) {
      const item = new ReportGroupItem(itemEntity)
