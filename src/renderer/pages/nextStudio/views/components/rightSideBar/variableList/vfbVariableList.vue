...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      this.varTreeData = [this.$store.getters.symbolVarTreeCfg]
      this.$nextTick(() => {
    ...
    bindSortable () {
      this.$nextTick(() => {
        // FIXME 要区分页面组
        const eles = this.$el.querySelectorAll('.VarTreePage')
        if (R.isNotEmpty(eles)) {
          eles.forEach(ele => {
            ...
            const el = ele.parentNode.parentNode.querySelector('div[role="group"]')
            if (el) {
              const node = this.$refs.tree.getNode(id)
              createSortable(el, node.data, 'VarTreeSymbol', this)
            }
          })
        }
      })
    ...
    dblClkHandler (node, data) {
      ...
      if (data instanceof VarTreeSymbol) {
        this.$vbus.$emit('VAR_TREE_SYMBOL_DBL_CLICKED', data)
      }
    }
  },
  mounted () {
